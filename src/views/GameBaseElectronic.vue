...
import { ACTIONS, VIEWS } from '@/logic/utils/Parameter'
import { useRouter } from 'vue-router'
import Tools from '@/logic/utils/Tools'
import ProgressBar from '@/components/Ui/ProgressBar.vue'

defineOptions({
  name: "GameBaseElectronic",
  inheritAttrs: false,
})
...
const loadGameLogic = (_game: any) => {
  return _game
}
defineExpose({ loadGameLogic })

var selfGame: any | null = null
// CLICKLISTENER.事件
const clickListener = (actions: string) => {
  ...
  switch (actions) {
    ...
    case ACTIONS.START:
      ...
      if (props.game === undefined || props.args === undefined) return
      //
      animationStatus('waittingPage', 'page--alphaOut', 100)
      setTimeout(() => {
        state.gameStatus = GAMESTATUS.READY
...
const changeGameStatus = (gameStatus: string) => {
  switch (gameStatus) {
    ...
    case GAMESTATUS.START:
      ...
      const GameClass: any = props.game
      selfGame = new GameClass(state)
      if (selfGame === null) return
...
<template>
  <div class="game-base game-base-electronice animations">
    <div id="container" class="game-container">
      ...
      <div id="waittingPage" class="waitting-status all-mask" v-if="state.viewWaitingVisible">
        ...
        <div class="nvbar">
          <div class="button back">
            <span class="
              self button--twirl
            " data-splitting @click="clickListener(ACTIONS.BACK)">BACK</span>
          </div>

          <div class="button settings">
            <span id="settingsBtn" class="icon-btn" @click="clickListener(ACTIONS.SETTINGS)">
              <svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="23px" height="23px"
                viewBox="0 0 1269.000000 1280.000000" preserveAspectRatio="xMidYMid meet">
                <metadata>
                  Created by potrace 1.15, written by Peter Selinger 2001-2017
                </metadata>
                <g transform="translate(0.000000,1280.000000) scale(0.100000,-0.100000)" fill="#00ff00" stroke="none">
        ...
        <div id="waittingPageMain" class="main">
          ...
          <div class="button-bar">
            <button id="startBtn" @click="clickListener(ACTIONS.START)">START</button>
          </div>
        </div>
        <!-- MAIN (settings-page) -->
        <div id="settingsPageMain" class="main settingsPage page--hide">
          ...
          <div class="header special-box">
            <!-- 標題1 -->
            <div class="title-block">
              <div id="settingsAnim01" class="title headline self" data-splitting> SETTINGS </div>
            </div>
            <!-- 標題2 -->
            <div class="title-second-block">
          ...
          <div class="button-block">
            <button id="saveBtn" class="button-save" @click="clickListener(ACTIONS.SAVE)">SAVE</button>
          </div>
        </div>
        ...
        <div id="detailsPageMain" class="main detailsPage page--hide">
          ...
          <div class="header special-box">
            ...
            <!-- content -->
            <div class="title-second-block">
              <div id="anim02" class="title2 content-block headline self headline--self001 " data-splitting> In the
                game,
                the player controls the snake, </div>
              <div id="anim02" class="title2 headline self headline--self001 " data-splitting> which will keep moving
                forward, picking up objects it touches along the way,</div>
              <div id="anim02" class="title2 headline self headline--self001 " data-splitting> and must avoid touching
                itself or other obstacles. </div>

              <div id="anim02" class="title2 content-block headline self headline--self001 " data-splitting> Every time
                the greedy snake eats a piece of food, </div>
              <div id="anim02" class="title2 headline self headline--self001 " data-splitting>its body grows a little.
              </div>
            </div>
          </div>
          <!-- BUTTON -->
        ...
        <div id="controlsPageMain" class="main controlsPage page--hide">
          ...
          <div class="header special-box">
            ...
            <!-- content -->
            <div class="title-second-block">
              <div id="contentAnim01" class="title2 content-block margin-top50 headline self" data-splitting> Hit the
                <span class="red">W</span> key on the keyboard to make the snake turn upward.
              </div>
              <div id="contentAnim02" class="title2 content-block headline self" data-splitting> Hit the <span
                  class="yellow">S</span> key on the keyboard to make the snake turn downward.</div>
              <div id="contentAnim03" class="title2 content-block headline self" data-splitting> Hit the <span
                  class="orange">A</span> key on the keyboard to make the snake turn to the left. </div>
              <div id="contentAnim04" class="title2 content-block headline self" data-splitting> Hit the <span
                  class="blue">D</span> key on the keyboard to make the snake turn to the right. </div>
            </div>
          </div>
          <!-- BUTTON -->
          <div class="button-bar">
        ...
        <div id="rankingPageMain" class="main rankingPage page--hide">
          <div class="header special-box">
            ...
            <!-- content -->
            <div class="title-second-block">
              <div class="ranking-block" :class="colorArr[index]" v-for="(item, index) in state.data.ranking"
                :key="index">
        ...
        <div class="footer">
          ...
          <div id="footer" class="
            content
            headline self
          " data-splitting> {{ props.data.time || 'TONY.WORKS@DEFAULT' }} </div>
        </div>
      </div>
      <!-- READY狀態  -->
      ...
      <div class="result-status all-mask" v-show="state.viewResultVisible">
        <div class="main">
          <div class="header special-box">
            ...
            <div id="resultStatusTitleAnim02" class="title2 headline self" data-splitting> CLICK AGAIN CAN RESTART PLAY
            </div>
          </div>
          ...
          <div class="button-bar result-bar">
            ...
            <button id="exitGameBtn" class="exit-button" @click="clickListener(ACTIONS.EXIT)">EXIT</button>
          </div>
      ...
      <!-- __GAME主體__ -->
      <div class="electronice-container">
        <div class="game-center">
            ...
            <div class="right-block">
              ...
              <div id="gameControlsBlock" class="controls-block page--hide">
                <div class="content">
                  ...
                  <div class="main-block">
                    <div class="controls-btn up" :class="[state.controlStatus === ACTIONS.CONTROLSUP ? 'isSelect' : '']"
                      @click="clickListener(ACTIONS.CONTROLSUP)">UP</div>
                    <div class="controls-btn left"
                      :class="[state.controlStatus === ACTIONS.CONTROLSLEFT ? 'isSelect' : '']"
                      @click="clickListener(ACTIONS.CONTROLSLEFT)">LEFT</div>
                    <div class="controls-btn down"
                      :class="[state.controlStatus === ACTIONS.CONTROLSDOWN ? 'isSelect' : '']"
                      @click="clickListener(ACTIONS.CONTROLSDOWN)">DOWN</div>
                    <div class="controls-btn right"
                      :class="[state.controlStatus === ACTIONS.CONTROLSRIGHT ? 'isSelect' : '']"
                      @click="clickListener(ACTIONS.CONTROLSRIGHT)">RIGHT</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="title-block">
            <h1 id="gameTitleAnim" class="headline self" data-splitting> {{ props.data.title || 'GAME-DEFAULT' }} </h1>
          </div>
          ...
          <div class="footer"> {{ props.data.game.footer || 'DEFAULT.GAME @ ...' }} </div>
        </div>
      </div>
      <!-- __GAME主體.END__ -->
    </div>
...
@import "../assets/css/gameBase/electronice.scss";
